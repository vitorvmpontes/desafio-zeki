...
- `aggregate.py` — agrega o dataset limpo em **dois níveis**: primeiro evento → conjunto × mês (poucas centenas de milhares de linhas; `aggregate_conjunto_mes`, combinável entre lotes via `combine_conjunto_mes`), depois faz o fan-out para município (`etl.ibge.fanout_municipio`) e agrega de novo para o grão **município × mês** (`aggregate_municipio_mes`), com indicadores próprios (`fec_aprox`, `dec_aprox_horas`) e contagem ponderada de causas por origem. Fazer o fan-out direto no nível evento (~19 milhões de linhas) estourou a memória de uma máquina comum ao rodar contra o dataset real -- ver docstring do módulo e `docs/DEVLOG.md`.
- `pipeline.py` — orquestra tudo: lê os Parquet baixados **em lotes** (nunca os ~19 milhões de eventos de uma vez -- ver `docs/DEVLOG.md`), limpa e agrega cada lote, combina os parciais, cruza com o bridge, e escreve `data/processed/municipio_mes.{parquet,csv}`. O tamanho do lote é ajustável com `--batch-size` (padrão 300 mil linhas) -- reduza se a máquina tiver pouca memória.
