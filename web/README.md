...
Painel web do Continua, em tema escuro, dividido em páginas focadas: ranking de municípios por risco de interrupção de energia, a tela de detalhe de cada município (histórico real vs. previsão, causas dominantes, importância das features), três páginas de apoio à decisão para o gestor de manutenção da distribuidora -- Priorização ("onde agir agora"), Tendências ("para onde está indo") e Geografia ("onde acontece de verdade", com um mapa real de clusters gerado a partir de um arquivo KML) -- e um Chat que traduz perguntas em português para SQL de verdade contra os dados (text-to-SQL via Google Gemini, ver `api/README.md`). Consome exclusivamente a API (`api/`) — não fala com o Postgres nem com os artefatos de ML diretamente.

## Stack

- **React + Vite + TypeScript** — escolhido com o usuário no início do Dia 6 (opção recomendada).
- **react-router-dom** — roteamento client-side, sete rotas: `/` (ranking), `/municipios/:codigoIbge` (detalhe), `/priorizacao` (ação imediata), `/tendencias` (piorando/melhorando + calendário sazonal), `/geografia` (mapa + hotspots + MTTR por região) e `/chat` (perguntas livres em português, traduzidas para SQL).
...
│   │   └── Chat.tsx              chat text-to-SQL: pergunta em português -> SQL real -> tabela de resultado (POST /chat)
...
- **A página de Priorização substitui a antiga página de Análises** (ver `docs/DEVLOG.md`, pivot "Priorização") -- o usuário avaliou que Análises descrevia o dataset nacional, mas não ajudava a decidir onde agir para mitigar quedas de energia. Priorização é pensada para o gestor de manutenção da distribuidora, com ranking por **impacto esperado** (não só taxa de risco isolada) e **ação recomendada** por município a partir do padrão de frequência x duração comparado ao percentil nacional (mais a causa reportada, quando existir, como evidência adicional na frase) -- ver `ml/priorizacao.py` e `api/README.md`. A tabela mostra dois selos por linha: o **padrão** (ex.: "Muitos eventos curtos", "Dentro do padrão") e a **confiança** da recomendação -- trocado de base depois que o usuário reportou que a versão anterior (baseada só em causa) repetia o mesmo texto em toda linha visível, ver `docs/DEVLOG.md`, "ação recomendada baseada em causa não ajudava".
...
- **`Chat.tsx` mantém um histórico local de trocas (pergunta + resultado/erro por mensagem, `useState`), não uma sessão de conversa no backend** -- cada pergunta é uma chamada independente a `POST /chat` (sem memória de perguntas anteriores no servidor); o "histórico" que a página mostra é só de exibição, no navegador. Cada mensagem tem seu próprio estado (`carregando`/`sucesso`/`erro`), então enviar uma pergunta nova não bloqueia nem afeta a anterior. O SQL gerado vem sempre visível por trás de um `<details>` ("Ver SQL gerado") -- transparência sobre o que de fato foi executado, sem poluir a resposta por padrão. Erros de configuração (503, "Chat não configurado") e de validação (422, consulta reprovada) aparecem como texto na própria bolha de resposta, sem distinção visual proposital entre os dois -- para o usuário do painel, a ação é a mesma nos dois casos (reformular a pergunta ou avisar quem administra a instância).
...
A página de Chat (`/chat`) foi validada com Playwright em dois estados reais: (1) sem `DATABASE_URL_READONLY` configurada (o estado em que este repositório é entregue por padrão) -- clicar numa pergunta de exemplo mostra a bolha de erro "Chat não configurado" corretamente, sem quebrar a página; (2) com um Postgres real (`continua_readonly`) e `api/chat_sql.py::gerar_sql` mockado (a chamada de rede ao Gemini é bloqueada neste sandbox de desenvolvimento, mesma limitação já documentada para ANEEL/Docker Hub) devolvendo uma consulta SQL fixa -- o fluxo completo depois disso é real: validação, execução contra os 131.793 dados reais, tabela renderizada com os dados corretos, e o `<details>` "Ver SQL gerado" mostrando a consulta de fato executada. A tradução de linguagem natural para SQL pelo Gemini em si (`gerar_sql`) só será validada de ponta a ponta na primeira execução na máquina do usuário, com uma `GEMINI_API_KEY` de verdade.
