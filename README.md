...
- [ ] Pipeline de ingestão e limpeza dos dados da ANEEL (`etl/`)
...
O Radar de Continuidade ingere os dados da ANEEL mensalmente, calcula indicadores de continuidade por município, e usa um modelo de aprendizado de máquina para prever o risco de interrupção do mês seguinte — expondo tudo isso por uma API e um painel simples, com a explicação (causas dominantes) por trás de cada previsão.
...
Isso sobe o banco Postgres usado pelo pipeline e pela API. Os demais serviços (`etl`, `api`, `web`) serão adicionados aos comandos acima conforme forem implementados — acompanhe o checklist no topo deste README e o [`docs/DEVLOG.md`](docs/DEVLOG.md).
...
├── etl/             ingestão e limpeza dos dados da ANEEL
...
└── data/            dados (raw/processed ignorados pelo git; sample versionada)
```
